...
2.7.2018
 	matの初期化のために，デザイン行列としてのランダマイズを導入した．
 	つまり，列ベクトルごとにランダマイズする．
...
void vect::rand(vect& mean, mat& sigma){
	if (size != mean.size){
		cout << "ERROR(vect.rand): invalid size of mean vect" << endl;
		return;
	}else if (size != sigma.rsize || sigma.rsize != sigma.csize){
		cout << "ERROR(vect.rand): invalid size of sigma" << endl;
		return;
	}
	...
	mt19937 mt(rnd());
	normal_distribution<> gauss(0.0, 1.0);   // 平均0.0、標準偏差1.0で分布させる
	for(int i=0; i<size; i++) v[i] = gauss(mt);
	(*this) = mean + (sigma * (*this));
}
...
void mat::rand(vect& mean, mat& sigma){
	//行ベクトルごとに多次元正規分布に従うよう初期化
	if (csize != mean.size){
		cout << "ERROR(mat.rand): invalid size of mean vect" << endl;
		return;
	}else if (csize != sigma.rsize || sigma.rsize != sigma.csize){
		cout << "ERROR(mat.rand): invalid size of sigma" << endl;
		return;
	}
	...
	for(int i = 0; i < rsize; i++){
		rn.rand(mean, sigma);
		for(int j = 0; j < csize; j++) a[i * csize + j] = rn.v[j];
	}
}
...
void mat::eigendecomp(vect *lambda, mat *U){
	//For given A, give the decomp as "A = U diag(l) U^T".
	if(rsize != csize){
...
//for design matrix
vect* mat::samp_mean(){
	//行ベクトルを一つのデータと見なして列に関し平均をとる
	vect* trg = new vect(csize);
	for(int i = 0; i < rsize; i++){
		*trg = *trg + this->row(i);
	}
	trg->scale(1.0/rsize);
...
mat* mat::centering(){
	//各行ベクトルから平均を引く
	vect mean; mean = samp_mean();
	mat* trg = new mat(); trg->copy(*this);
	for(int i = 0; i < rsize; i++){
		for(int j = 0; j < csize; j++){
			trg->a[i * csize + j] -= mean.v[j];
		}
	}
	return trg;
}
mat* mat::samp_var(){
	//行ベクトルを一個のデータとして分散をとる
	mat centered; centered = centering();
	mat* trg = centered.transpose();
	(*trg) = (*trg) * centered;
	trg->scale(1.0/rsize);
	return trg;
}
...
void sort_with_id(double* src, int len_src, int* trg, int len_trg){
	//sort the list `src' in descending order
	//	and output the sorted indeces `trg'
	double* max = dcalloc(len_trg);
	for(int i = 0; i < len_trg; i++) max[i] = -10000.0;
	
	for(int i = 0; i < len_src; i++){
		for(int k = 0; k < len_trg; k++){
			if(src[i] > max[k]){
				for(int j = k; j < len_trg - 1; j++){
					max[j+1] = max[j]; trg[j+1] = trg[j];
				}
				max[k] = src[i]; trg[k] = i;
				break;
			}
